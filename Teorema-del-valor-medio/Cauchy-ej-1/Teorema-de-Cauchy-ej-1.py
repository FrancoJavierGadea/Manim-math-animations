...
class MainScene(Scene):
  ...
  def construct(self):
    ...
    # Funcion
    fxTex = MathTex(r'f(x) = \frac{(x-6)^{2}}{3} + \frac{5}{2}', font_size=37, color=WHITE)
    fxTex.move_to(axis.c2p(10.5, 8) + fxTex.get_critical_point(DR) - fxTex.get_critical_point(ORIGIN))


    gxTex = MathTex(r'g(x) = -\frac{(x-7)^{2}}{3} + \frac{11}{2}', font_size=37, color=WHITE)
    gxTex.move_to(axis.c2p(10.5, 4) + gxTex.get_critical_point(DR) - gxTex.get_critical_point(ORIGIN))


    def fx(x):
      return -math.pow(x - 7, 2) / 3 + 5.5

    def gx(x):
      return math.pow(x - 6, 2) / 3 + 2.5
    ...
    self.play(
        ...
        FadeIn(pointR2),
        Create(getLineToAxisX(pointR2, pointR2.get_color(), hide_line=True)),
    )
