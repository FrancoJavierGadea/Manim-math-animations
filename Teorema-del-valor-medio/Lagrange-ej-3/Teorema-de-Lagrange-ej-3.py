...
class MainScene(Scene):
  ...
  def construct(self):
    ...
    # Funcion
    fxTex = MathTex(r'f(x) = \sin(2 \cdot x) + 3', font_size=40, color=WHITE)
    fxTex.move_to(axis.c2p(1, 7.7) + fxTex.get_critical_point(DR) - fxTex.get_critical_point(ORIGIN))
    ...
    lineR2 = modelLine.copy().move_to(pointR2.get_center())
    ...
    self.play(
        FadeIn(pointR1),
        Create(getLineToAxisX(pointR1, pointR1.get_color(), 'c_{1}')),
        FadeIn(pointR2),
        Create(getLineToAxisX(pointR2, pointR2.get_color(), 'c_{2}')),
    )
