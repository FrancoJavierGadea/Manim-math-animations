...
EQ_1 = [
  MathTex(r'\sin(-\alpha) = -\sin(\alpha)', font_size = 40, color = WHITE),
  MathTex(r'\cos(-\alpha) = \cos(\alpha)', font_size = 40, color = WHITE),
  MathTex(r'\tan(-\alpha) = -\tan(\alpha)', font_size = 40, color = WHITE),
]
...
class MainScene(Scene):
  ...
  def construct(self):
    ...
    def getLines(angle, axis, sin_color = WHITE, cos_color = WHITE, angle_color = WHITE,  text = ''):
      ...
      group.angle_text = MathTex(text, font_size=38, color=WHITE, z_index = 12)

      if(angle < 0):

        group.angle_arc.rotate(180 *  DEGREES, [1, 0, 0], axis.get_origin())
        group.angle_bg.rotate(180 *  DEGREES, [1, 0, 0], axis.get_origin())


      if(angle < 0):
        group.angle_text.next_to(axis.get_origin() + [0.65, -0.2, 0], DR, buff = 0)
      else:
        group.angle_text.next_to(axis.get_origin() + [0.65, 0.2, 0], UR, buff = 0)
