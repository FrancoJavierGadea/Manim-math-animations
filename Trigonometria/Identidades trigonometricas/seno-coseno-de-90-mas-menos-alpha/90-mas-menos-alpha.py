...
EQ_1 = [
  MathTex(r'\sin(90^{\circ} + \alpha) = \cos(\alpha)', font_size = 40, color = WHITE),
  MathTex(r'\cos(90^{\circ} + \alpha) = -\sin(\alpha)', font_size = 40, color = WHITE),
  MathTex(r'\tan(90^{\circ} + \alpha) = -\dfrac{1}{\tan(\alpha)}', font_size = 40, color = WHITE),
]
...
class MainScene(Scene):
  ...
  def construct(self):
    ...
    BetaLines = always_redraw(
      lambda: getLines(
        angle = (alfa.get_value() + 90),
        axis = axisBeta,
        text = r'90^{\circ} + \alpha',
        sin_color = RED,
        cos_color = YELLOW,
        angle_color = PURPLE
      )
